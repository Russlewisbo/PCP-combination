...
| 2026-08-07 | Russ + Assistant | Added absolute-effect + secondary analyses. Bayesian page now shows **risk difference −6.2% (95% CrI −21% to +9%, point NNT ≈16)** and a **95% prediction interval OR 0.24–2.43** (a new study's plausible effect spans benefit→harm). New **clinical-response** section: pooled OR **1.87 (0.78–4.46)** clean / **2.19 (0.95–5.24)** incl. confounded Li2024a (its 20/20 → OR ≈21 outlier flagged; same one that inflated Yang). **Adjusted estimates NOT poolable** — only Jin2019 (all-pt adj HR 1.10, ~null) and Xu2025 (IPTW/Cox HR 2.97) report one, and they conflict. **Mechanical ventilation NOT pooled** — source check found the MV figures are baseline/course-of-illness severity, not cleanly-timed incident events (Qi2025/Xu2025 unconfirmed). Surfaced response + prediction interval on Conclusions page. | Headline unchanged: sensitivity OR 0.76 (0.43–1.37), inconclusive. Published to gh-pages (e25e5cb). Pick up at "Next steps". |
| 2026-09-05 | Russ + Assistant | Reconciled PRISMA against the Covidence funnel (172 screened → 52 full-text → 48 assessed → 21 studies/22 reports; the Paulina RIS holds **172** records despite its "177" filename); built the canonical **PRISMA2020-package** flow (`analysis/prisma2020_flow.R` + `prisma2020_data.csv`) alongside the custom analytic flow. Replaced the JAGS MCMC page with a dependency-free **base-R random-walk Metropolis sampler** (`fit_mcmc`; JAGS/rjags no longer installed). Added **300 dpi figure exports** (`analysis/export_figures.R`) and a **goodness-of-fit** page (influence/Baujat + posterior predictive checks: all studies within PP intervals, omnibus Q ppp = 0.40 → adequate fit; Qi2025 the influential outlier). **Descoped NMA → pairwise** (PRISMA MA box 6/6). Added **GRADE SoF** (⊕◯◯◯ very low) on Conclusions. Fixed the publish-timeout cause: slow `rposterior(predict=TRUE, n=2e4)` (~6 min/page) → analytic `qposterior`. Expanded **ROBINS-I** into a documented per-domain draft (kept preliminary). Response outcome: heterogeneity driven entirely by confounded Li2024a (clean k=2 I²=0; add=0 → its OR undefined) — inconclusive. **Timing is NOT an effect modifier** (moderator QM(1)=0.00, p=0.99); reconciled the old "timing explains the gap" note across memory, Sensitivity, and Conclusions. | Site live on gh-pages; headline unchanged (sensitivity OR 0.76, 0.43–1.37). RoB still preliminary (needs human dual ROBINS-I). Pick up at "Next steps". |
